...
def get_img_path_batches(batch_size: int, img_dir: str):
    ret = []
    batch = []
...
def plot_one_box(x, img: np.ndarray, color=None, label=None, line_thickness=None):
    """
...
class YoLov5TRT(Thread):
    """
    description: A YOLOv5 class that warps TensorRT ops, preprocess and postprocess ops.
    """

    def __init__(self, engine_file_path: str, plugin: str, categories: List[str]):
        # load plugin
        ctypes.CDLL(plugin)

        # Create a Context on this device,
        super().__init__()
        self.ctx = cuda.Device(0).make_context(cuda.ctx_flags.SCHED_AUTO)
        stream = cuda.Stream()
    ...
    def infer(self, raw_image_generator):
        ...
        for index, image_raw in enumerate(raw_image_generator):
            input_image, image_raw, origin_h, origin_w = self.preprocess_image(image_raw)
            batch_image_raw.append(image_raw)
            batch_origin_h.append(origin_h)
            batch_origin_w.append(origin_w)
            np.copyto(batch_input_image[index], input_image)
        ...
        for index in range(self.batch_size):
            result_boxes, result_scores, result_classid = self.post_process(
                output[index * 6001: (index + 1) * 6001], batch_origin_h[index], batch_origin_w[index]
            )
